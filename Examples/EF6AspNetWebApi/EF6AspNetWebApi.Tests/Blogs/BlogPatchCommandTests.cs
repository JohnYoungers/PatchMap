...
namespace EF6AspNetWebApi.Tests.Blogs
{
    ...
    public class BlogPatchCommandTests : BaseTest
    {
        ...
        public void Updates()
        {
            ...
            var blog = new BlogGetCommand(dbContext).Execute(dbBlog.BlogId);
            blog.Name = blog.Name + "Updated";
            blog.Tags = new List<string> { "A", "D" };
            blog.PromotedPost = new PostSummaryViewModel { Id = dbBlog.Posts[0].PostId };

            var results = new BlogPatchCommand(dbContext).Execute(dbBlog.BlogId, blog.ToPatchOperations());
